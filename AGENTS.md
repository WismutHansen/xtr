...
**IMPORTANT**: This project uses **trx** for ALL issue tracking. Do NOT use markdown TODOs, task lists, or other trackers.
...
**Create new issues:**
```bash
trx create "Issue title" -t bug|feature|task|epic|chore -p 0-4
```
...
**Complete work:**
```bash
trx close <id> -r "Completed"
```

### Workflow for AI Agents

1. **Check ready work**: `trx ready`
2. **Claim task**: `trx update <id> --status in_progress`
3. **Implement + test**
4. **If new follow-up work appears**: create a new trx issue and reference parent in description
5. **Complete**: `trx close <id> -r "Done"`
6. **Commit tracking metadata with code**: include `.trx/` changes in the same commit

### Important Rules

- ✅ Use trx for all tracking
- ✅ Update status as you work (`open` -> `in_progress` -> `closed`)
- ✅ Keep `.trx/` committed with related code changes
- ❌ Do not create markdown TODO trackers
- ❌ Do not use parallel tracking systems
